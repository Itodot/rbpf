    ...
    while insn_ptr * ebpf::INSN_SIZE < prog.len() {
        ...
        match insn.opc {
            ...
            ebpf::LD_H_REG => {
                reg[_dst] = unsafe {
                    let x =
                        (reg[_src] as *const u8).wrapping_offset(insn.off as isize) as *const u16;
            ...
            ebpf::LD_W_REG => {
                reg[_dst] = unsafe {
                    let x =
                        (reg[_src] as *const u8).wrapping_offset(insn.off as isize) as *const u32;
            ...
            ebpf::LD_DW_REG => {
                reg[_dst] = unsafe {
                    let x =
                        (reg[_src] as *const u8).wrapping_offset(insn.off as isize) as *const u64;
                    check_mem_load(x as u64, 8, insn_ptr)?;
                    x.read_unaligned()
                }
            }
            ...
            ebpf::JA => do_jump(),
            ebpf::JEQ_IMM => {
                if reg[_dst] == unsigned_u64!(insn.imm) {
                    do_jump();
                }
            }
            ebpf::JEQ_REG => {
